...
def overlap_crop_forward(ms_up, ms, pan, model, shave=16, min_size=68200, bic=None):
    """
    ...
    if w_size * h_size <= min_size:  # 如果已经小于最小尺度
        ...
        sr_list = [
            overlap_crop_forward(ms_up_p, ms_p, pan_p, model, shave=shave, min_size=min_size) \
...
def test(test_data_loader, model, sate):
    ...
    count = 0
    for ms_up_crop, ms_org_crop, pan_crop, _, _ in val_bar:
        ...
        with torch.no_grad():  # validation
            ...
            start = time.time()
            ### start testing ######
            out = overlap_crop_forward(ms_up, ms, pan, model, shave=4, min_size=72000, bic=None)  # 62000, or crop 1/4 :72000, crop 1/2 :271000
...
if __name__ == "__main__":
    ...
    parser.add_argument('--testBatchSize', type=int, default=20, help='testing batch size')
    ...
    if opt.satellite == 'ik':
        ...
        opt.checkpoint = 'wv3_8_epoch_010.pth'
