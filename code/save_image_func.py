...
def save_image_RS(sate, file_name, fused):
    ...
    if sate == 'wv3_8':
        save_path_fused = join(r'fused/WorldView-3-8-60ep-1wind/wv3-8b', file_name)    ## need to change here
        save_path_rgb = join('fused/WorldView-3-8-60ep-1wind/wv3-8-rgb', ''.join([file_name[:-4], '.tif'])) ## need to change here
    ...
    if sate == 'ge':
        save_path_fused = join(r'fused/GeoEye_1/geo_4b', file_name)
        save_path_rgb = join('fused/GeoEye_1/geo_rgb', ''.join([file_name[:-4], '.tif']))
    if sate == 'qb':
        save_path_fused = join(r'fused/QuickBird/qb_4b', file_name)
        save_path_rgb = join('fused/QuickBird/qb_rgb', ''.join([file_name[:-4], '.tif']))
    if sate == 'ik' or sate == 'pl' or sate == 'qb' or sate == 'ge':
...
def save_image_full(sate, file_name, fused):
    ...
    if sate == 'wv3_8':
        save_path_fused = join(r'fused/WorldView-3-8-noTF20ep/wv3-8b-FS', file_name)
        save_path_rgb = join('fused/WorldView-3-8-noTF20ep/wv3-8-rgb-FS', ''.join([file_name[:-4], '.tif']))
        # save_path_ms_up = join(r'../fused\WorldView-3-8\wv3-8-up', file_name)
